...
# Extract raceId and milliseconds columns
...
"""
average_list_seconds = np.divide(average_list,1000)
coefficients = np.polyfit(list_of_races, average_list_seconds, 1)  # 1 for a linear trendline
p = np.poly1d(coefficients)
plt.plot(list_of_races,p(average_list_seconds),"r--")
plt.bar(list_of_races,average_list_seconds)
xlabel('Race ID')
ylabel('Time (s)')
...
m, b = coefficients
print(f"Trendline equation: y = {m:.2f}x + {b:.2f}")

"""
Question 2: What is the fastest pit stop time overall?
"""

fastest_race_time = np.min(pit_race_id_numpy[:,1]) / 1000

print("The fastest pit stop time from 2011 to 2024 was " + str(fastest_race_time) + " seconds")

"""
Question 3: Which race ID had the fastest pit stop time?
"""

fastest_race_index = np.argmin(pit_race_id_numpy[:,1])
fastest_race_id = pit_race_id_numpy[fastest_race_index,0]

print("The race ID with the fastest pit stop is " + str(fastest_race_id))
...
fastest_lap_time = np.min(lap_time_list) / 1000
print("The fastest lap time for the Chinese Grand Prix is " + str(fastest_lap_time) + " seconds")

"""
Question 5: How has lap time decreased over time for a selected circuit?
"""
...
average_CGP_seconds = np.divide(average_CGP_time_list,1000)
...
driver_winner = stats.mode(driver_winner_list)
print(driver_winner)
print("The driver with the most wins ever is Driver ID 1 with 125 wins. This is Lewis Hamilton")
...
constructor_winner = stats.mode(constructor_win_list)
